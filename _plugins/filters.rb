...
module Jekyll
  module CustomFilters
    ...
    def to_xml_feed(input)
      ...
      "<item><title>#{title}</title><link>#{Jekyll.configuration({})['url']}/api/change-log.html</link><pubDate>#{date}</pubDate><guid isPermaLink=\"true\">#{Jekyll.configuration({})['url']}/api/change-log.html##{id}</guid><description><![CDATA[#{content}]]></description></item>"
    end

    def dot_sep_link(name, *parts)
      path = parts.join('.')
      "<a href=\"#{(path +  ".html").gsub('[i]','_i_')}\">#{name}</a>"
    end
    
    def quote_strings(array)
      array.map { | e | (e.is_a? String) ? "\"#{e}\"" : e }
    end
